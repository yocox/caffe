...
DEFINE_string(working_dir, "",
    "Change working directory to, before start training");
...
int train() {
  ...
  CHECK_GT(FLAGS_working_dir.size(), 0) << "Need assign working directory to finetune.";
  std::cout << "Current working directory " << cwd() << std::endl;
  std::cout << "Change working directory to " << FLAGS_working_dir << std::endl;
  if(chdir(FLAGS_working_dir.c_str()) != 0) {
    LOG(ERROR) << "Can't change dir to '" << FLAGS_working_dir << "'";
    return 0;
  }
  std::cout << "Current working directory " << cwd() << std::endl;
